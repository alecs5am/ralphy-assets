...
All eleven projects are in `ugc-cli` under `workspace/projects/<id>/` with their full asset trees, gen-logs, scenarios, and prompts. The mp4s here are the **canonical final** picked at the end of each working session. All ten new/updated entries re-encoded at `libx264 preset=slow CRF=24, aac 128k, +faststart` (visually-transparent recompression) — sizes stay well under GitHub's 100 MB raw limit so files stream from `raw.githubusercontent.com` directly without Release upload.
